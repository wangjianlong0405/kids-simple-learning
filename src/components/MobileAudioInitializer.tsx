...
const MobileAudioInitializer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('audioInteractionDetected', handleUserInteraction as EventListener);

    return () => {
      window.removeEventListener('audioInteractionDetected', handleUserInteraction as EventListener);
    };
  }, []);
  ...
  const handleInitialize = async () => {
    ...
    try {
      // 尝试播放测试音频
      await mobileAudioHandler.playTTS('音频初始化成功！', {
        lang: 'zh-CN',
        rate: 0.8,
        pitch: 1.0,
        volume: 0.8
  ...
  if (!isVisible || !deviceInfo?.isMobile) return null;
          ...
          <div className="flex items-start space-x-4">
            ...
            <div className="flex-1">
              ...
              <div className="text-xs opacity-70 mb-4">
                <div>设备: {deviceInfo.isIOS ? 'iOS' : deviceInfo.isAndroid ? 'Android' : '移动端'}</div>
...
export default MobileAudioInitializer;
